<?php
	...
	function bug_create( $p_bug_data ) {
		...
		$c_os					= xdb_prepare_string( $p_bug_data->os );
		$c_os_build				= xdb_prepare_string( $p_bug_data->os_build );
			...
			for ( $i = 0; $i < $t_count; $i++ ) {
				...
				$query = "INSERT INTO $t_mantis_bug_file_table
						...
						VALUES ( '$t_new_bug_id',
								 ...
								 '" . xdb_prepare_string( $t_bug_file['file_type'] ) . "',
	...
	function bug_prepare_db( $p_bug_data ) {
		...
		$t_bug_data->os					= xdb_prepare_string( $p_bug_data->os );
		$t_bug_data->os_build			= xdb_prepare_string( $p_bug_data->os_build );
		$t_bug_data->platform			= db_prepare_string( $p_bug_data->platform );
		$t_bug_data->version			= xdb_prepare_string( $p_bug_data->version );
